...
//
// V2 (unlike the type-fixed-cumulative V1
// aclnnGroupedMatmulSwigluQuantWeightNZ) exposes an explicit groupListType, so
// we feed per-expert COUNTS directly and drop the host-side cumsum entirely. It
// takes NO swigluLimit argument (verified against both the CANN 9.0.0 built-in
// aclnn header and the ops-transformer op proto), so EXEC_NPU_CMD must pass
// exactly 17 args. Header on device declares:
//   GetWorkspaceSize(x, weight[list], weightScale[list],
//   weightAssistMatrix[list],
//                    bias, xScale, smoothScale, groupList,
//                    dequantMode, dequantDtype, quantMode, groupListType,
//                    tuningConfig, output, outputScale, ws, exec)
//
...
//   group_list   int64 (E,)       per-expert COUNTS (groupListType=1, no
//   cumsum)
//   -> y         int8  (M, N),    N = 2N / 2
